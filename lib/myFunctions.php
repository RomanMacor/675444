...
function getCategories($db){
	return $db->query("SELECT name FROM category");
}
/*
...
function showItems($result){
	...
	$buildHTML .= "<tr> <th> <a href='list.php?orderBy=id'> ID </a> </th> <th> <a href='list.php?orderBy=name'> Name </a> </th> <th>
					 ...
					 <th> <a href='list.php?orderBy=category'> Category </a> </th> <th>Description</th></tr>";
...
function showCategories($result){
	...
	if($result){
		while($row = $result->fetch_object()){
			$buildHTML .= "<tr> <td> $row->id </td> <td> $row->name </td> <td> <a href='deleteCategory.php?id=$row->id'> Delete </a> </td> </tr> ";
		}
		echo $buildHTML;
	}
}
